...
public class CustomerRegistration
{
    ...
    {
        string result = await _client.GetStringAsync("/api/Customers/" + customerId);
    }

    private async Task GetAccount(string accountId)
    {
        string result = await _client.GetStringAsync("/api/Accounts/" + accountId);
    }
    ...
    private async Task Deposit(string account, double amount)
    {
        ...
        var response = await _client.PatchAsync("api/Accounts/Deposit", content);
    ...
    private async Task Withdraw(string account, double amount)
    {
        ...
        var response = await _client.PatchAsync("api/Accounts/Withdraw", content);
        string result = await response.Content.ReadAsStringAsync();
    ...
    {
        var response = await _client.DeleteAsync("api/Accounts/" + account);
        response.EnsureSuccessStatusCode();
    }
}
